...
exports.root = (req, res) => {
    Customer.find()
    .then(customers => {
        res.render('mobile_phones_view',{
            results_customers: customers
          });
    }).catch(err => {
        res.status(500).send({
            message: err.message || "An error occurred while retrieving all Customers."
...
// search for Customers, matching string on firstName field
...
exports.searchSurname = (req, res) => {
    var search = req.params.s;
    console.log("Searching Costumers with Surname: "+search)
    Customer.find({ surname: new RegExp(search,"ig")})
    .then(customers => {
        res.render('mobile_phones_view',{
            results_customers: customers
          });
